...
export const GET = async (req) => {
  try {
    ...
    if (!userId || !date) throw new Error("Missing Data");

    const user = await User.findById(userId);
    if (!user) throw new Error("User not found");

    if (user.manager != jwtData.id) throw new Error("Unauthorized");
    ...
    if (user?.blockDate) {
      ...
      if (blockDateBD.isBefore(currentDateBD)) {
        throw new Error(
          `User is Blocked from ${new Date(user.blockDate).toDateString()}`
        );
      }
    }
